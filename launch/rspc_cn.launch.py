    ...
    if use_config_file:
        ...
        if not camera_config_path:
            ...
            if camera_config_file in available_configs:
                ...
                return [
                    LogInfo(
                        msg="ERROR: unknown config: '{}' \
                            - Available configs are: {} - not starting Realsense Polled Camera".format(
            ...
            if not os.path.isfile(camera_config_path):
                return [
                    LogInfo(
                        msg="ERROR: config_path file: '{}' \
                            - does not exist. - not starting Realsense Camera".format(
                            camera_config_path
                        )
                    )
                ]
